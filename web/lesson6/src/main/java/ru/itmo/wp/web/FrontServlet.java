...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        Method before = findMethod(pageClass, Route.BEFORE_ACTION);
        Method action = findMethod(pageClass, route.getAction());
        Method after = findMethod(pageClass, Route.AFTER_ACTION);

        if (before == null || action == null || after == null) {
            throw new NotFoundException();
        }
        ...
        for (Method method : new Method[]{before, action, after}) {
            try {
        ...
        try {
            ...
            throw new ServletException("Can't render template [pageClass=" + pageClass + ", method=" + action + "].", e);
        }
    }
