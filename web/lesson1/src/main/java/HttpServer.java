...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        if (file.isFile()) {
            final String prevETag = request.getHeader("If-None-Match");
            final String newETag = getETag(file);

            response.setHeader("ETag", newETag);
            if (newETag.equals(prevETag)) {
    ...
    private String getETag(File file) {
        return String.valueOf(file.length()) + (file.lastModified());
    }
